...
- retention-time units declared in the variable's `units` attribute, plus a
  decisive magnitude heuristic when the attribute is absent or unknown; an
  ambiguous unit refuses to convert (`--rt-unit` resolves it);
